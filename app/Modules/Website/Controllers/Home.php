...
class Home extends BaseController 
{
    ...
    ******************************/
    ...
    public function get_updadteOnwer11()
    {
        $nfts = $this->common_model->where_rows('nfts_store', array(), 'id', 'asc');
        $msg = ''; 
        if($nfts){
            $contractAdd      = $this->common_model->where_row('contract_setup', ['status'=>1]); 
            foreach ($nfts as $key => $value) { 
                ...
                // $check = $this->common_model->where_rows('nfts_store', ['token_id'=> $value->token_id ], 'id', 'asc'); 
                
                // if(count($check) > 1){
                //     echo $value->id." - ".$value->token_id;
                //     echo "<br/>";
                // }

                $result = $this->blockchain->getOwnerbyTokeid( $contractAdd->contract_address, $value->token_id ); 
                if($result){
                    if(isset($result->data->data)){
                        $ownerWallet = $result->data->data;
                        if($ownerWallet != $value->owner_wallet){
                            $user = $this->common_model->where_row('user', ['user_id'=> $ownerWallet ]); 
                            $this->common_model->update('nfts_store', ['id'=>$value->id], ['owner_wallet'=>$ownerWallet, 'user_id' => $user->user_id ]);
                            $this->common_model->update('nfts_store_log', [ 'store_id' => $value->id], ['owner_wallet'=>$ownerWallet, 'user_id' => $user->user_id ]);
                            $msg .= "success ".$ownerWallet; 
                            echo $value->id." - ".$value->token_id;
                            echo "<br/>";
                        }
                    } 
                    else{
                        $msg .= "Nft tokenID = ".$value->token_id." this transaction is not complete!";
                    }
                }
                else{
                    $msg .= "Please try again";
    ...
    public function get_updadteOnwerk()
    {
        $nfts = $this->common_model->where_rows('nfts_store', array(), 'id', 'asc');
        $msg = ''; 
        if($nfts){
            ...
            for ($a = 5562 ; $a> 0; $a--) { 
                ...
                $result = $this->blockchain->getOwnerbyTokeid( $contractAdd->contract_address, $a); 
                if($result){
                    if(isset($result->data->data)){
                        $ownerWallet = $result->data->data;
                        $this->common_model->update('nfts_store', ['file' => substr($ownerWallet,24) ], ['token_id'=>$a]);
                        $this->common_model->update('nfts_store_log', ['file' => substr($ownerWallet,24) ], ['token_id'=>$a]);
                    } 
                   
                    else{
                        $msg .= "Nft tokenID = ".$a." this transaction is not complete!";
                    }
                }
                else{
                    $msg .= "Please try again";
    ...
    public function get_updadteOnwerL()
    {
        $nftListings = $this->common_model->where_rows('nft_listing', array(), 'id', 'asc');
        // $nftListings = $this->common_model->where_rows('nft_listing_log', array(), 'id', 'asc');
        $msg = ''; 
        if($nftListings){
            foreach ($nftListings as $key => $value) { 
                $result = $this->common_model->where_row('nfts_store', ['id'=> $value->nft_store_id ]); 
                
                if($result){
                    echo $value->id."-".$result->token_id;
                    echo "</br>";
                    $this->common_model->update('nft_listing', ['id'=> $value->id], ['nft_token_id'=> $result->token_id, 'list_from' => $result->user_id]);
                    // $this->common_model->update('nft_listing_log', ['id'=> $value->id], ['nft_token_id'=> $result->token_id, 'list_from' => $result->user_id ]);
                }
                else{
                    $msg .= "Please try again";
                }
    ...
    // public function get_updadteOnwerSL()
    // {
    //     $nftListings = $this->common_model->where_rows('nfts_store_log', array(), 'id', 'asc');
    //     $msg = ''; 
    //     if($nftListings){
    //         foreach ($nftListings as $key => $value) { 
    //             $result = $this->common_model->where_row('nfts_store', ['id'=> $value->store_id ]); 
                
    //             if($result){
    //                 echo $value->id."-".$result->token_id;
    //                 echo "</br>";
    //                 $this->common_model->update('nfts_store_log', ['id'=> $value->id], ['token_id'=> $result->token_id]);
    //             }
    //             else{
    //                 $msg .= "Please try again";
    //             }
    ...
    // public function get_updadteOnwer1111()
    // {
    //     $model = 'nfts_store_log';

    //     $nftListings = $this->common_model->where_rows($model, array(), 'id', 'asc');
    //     $msg = ''; 
    //     if($nftListings){
    //         foreach ($nftListings as $key => $value) { 
    //             $result = $this->common_model->where_row('user', ['wallet_address'=> $value->owner_wallet ]); 
                
    //             if($result){
    //                 echo $value->owner_wallet;
    //                 echo "</br>";
    //                 $this->common_model->update($model, ['id'=> $value->id], ['user_id'=> $result->user_id]);
    //             }
    //             else{
    //                 $msg .= "Please try again";
    //             }
    ...
    public function get_updadteOnwer()
    {
        $model = 'nft_listing_log';
        ...
        if($nftListings){
            foreach ($nftListings as $key => $value) { 
                ...
                if($result){
                    echo $value->nft_store_id;
                    echo "</br>";
                    $this->common_model->update($model, ['id'=> $value->id], ['list_from'=> $result->user_id]);
                }
                else{
                    $msg .= "Please try again";
                }
