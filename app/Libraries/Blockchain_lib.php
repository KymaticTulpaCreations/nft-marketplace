...
class Blockchain_lib
{
    ...
    public function getOwnerbyTokeid($address, $tokeId)
    {
        if (empty($address)) {
            return $this->errorResponse("Transaction hash field is required");
        }
       
        $txhash = $this->curlExcute('api/transaction/getOwnerFromTokenId', ['contractAddress' => $address, 'tokenID'=> $tokeId , 'netWork'=>$this->network]);
        $txhash = $this->toJsonDecode($txhash);
        if ($txhash) {
            if($txhash->status=="success"){ 
                return $this->successResponse($txhash->msg, $txhash); 
