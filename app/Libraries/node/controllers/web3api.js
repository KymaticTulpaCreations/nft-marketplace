...
// import {utils, BigNumber, uint256} from "ethers";
...
export const getOwnerFromTokenId = async (req, res) => {
  try {
  const { contractAddress, netWork, tokenID } = req.body;
  const web3 = getWeb3(netWork);
   
  const contractBuf = new web3.eth.Contract(abi, (contractAddress).toLowerCase() );
  // const result = await contractBuf.methods.ownerOf(tokenID).call();
    const result = await contractBuf.methods.tokenURI(tokenID).call();
    if(result){
      return toSuccess(res, result);
    }else{
      return toError(res, 'Pending Get Address please wait!');
    }
