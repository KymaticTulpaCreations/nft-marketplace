...
async function mintToken(contractAddress = '', tokenURI = '', nftId = '') {
    ...
    try {
       ...
       // const provider = new ethers.providers.JsonRpcProvider("https://mainnet-rpc.5dax.com");
        await callBlockNumber(provider,item,nftId)


    } catch (error) {
        console.log("đ",error.message);
        let postData = {};
        postData[csrf_token] = get_csrf_hash;
...
async function callBlockNumber (provider, item,nftId){
    try {
        const block =  provider.getTransaction(item.hash);
        let tokenId  = 0;
        block.then(function(result) {
            console.log(result.to, result.blockNumber) // "Some User token"
            if(result.blockNumber){
                ...
                callPromise.then(function(result) {
                    tokenId = Number(result[0].topics[3]);
                    if(tokenId > 0){
                        if (item.hash) {
                            let postData = {};
                            postData[csrf_token] = get_csrf_hash;
                ...
                callBlockNumber(provider, item,nftId);
            }
        })
    } catch (error) {
...
async function transferNft(recieverAddress, tokenID, transferFee, contractAddress, nftId) {
    ...
    try {
        ...
        let txData = await contract.transferNFT(recieverAddress, tokenID, fees, { value: fees, gasLimit: 1000000 });
...
$("body").on("submit", "#list_for_sale_form", async function (event) {
    ...
    try {
        ...
        let list = await contract.listNFT(tokenID, ethers.utils.parseUnits(price, "ether"), { gasLimit: 1000000 });
